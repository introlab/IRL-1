...
/* Re-enable if we one day allow standalone motors:
namespace {
    static RCDeviceCreatorHelper ch_("PTUMotor", 
                                     RCDeviceCreator(&PTUMotor::create));
}

RCDevicePtr PTUMotor::create(const ros::NodeHandle& np)
{
    return RCDevicePtr(new PTUMotor(np));
}
*/

PTUMotor::PTUMotor(const ros::NodeHandle& n):
    RCDevice(n),
    cur_pos_(0.0), 
    cur_vel_(0.0), 
    cur_eff_(0.0),
    ...
    cycle_(0)
{
    ...
    ROS_INFO("Created PTU controller for %i:%i.", deviceID(), base_offset_);
}

void PTUMotor::registerCtrlIfaces(IRLRobot& robot)
{
...
void PTUMotor::enable(irl_can_bus::CANManager& can)
{
    ...
    ROS_INFO("Enabled PTU controller for %i:%i.", deviceID(), base_offset_);
}

void PTUMotor::enableCtrl(irl_can_bus::CANManager& can)
{
...
void PTUMotor::disableCtrl(irl_can_bus::CANManager& can)
{
...
void PTUMotor::disable(irl_can_bus::CANManager& can)
{
    CANRobotDevice::disable(can);
}

void PTUMotor::processMsg(const irl_can_bus::LaboriusMessage& msg)
{
    unsigned int dest = msg.msg_cmd - base_offset_;

    //ROS_DEBUG("PTUMotor::processMsg: state: %i, dest: %i, offset: %i", 
    ...
    switch (dest)
    {
        case CUR_POS_OFFSET:
            ...
            if (CANRobotDevice::state() == STATE_STARTING)
            {
                ...
                ROS_INFO("PTU controller state ready for %i:%i.",
                         deviceID(),
                         base_offset_);
            }
            break;
...
void PTUMotor::requestState(irl_can_bus::CANManager& can)
{
    ROS_DEBUG_THROTTLE(1.0, "PTUMotor request state for %i:%i", 
                       deviceID(),
                       base_offset_);
    if ((cycle_ % clock_divider_) == 0)
    {
        ...
        ROS_DEBUG_THROTTLE(1.0, "requestState sent for %i:%i", 
                           deviceID(), base_offset_);
    }

    // NOTE: cycle_ is only updated here:
    cycle_++;

}

void PTUMotor::sendCommand(irl_can_bus::CANManager& can)
{
    // NOTE: cycle_ is updated in requestState().
    if ((cycle_ % clock_divider_) != 0)
        return;
...
void PTUMotor::startMotor(irl_can_bus::CANManager& can)
{
    static unsigned char cmd = 1;
    can.writeMem(deviceID(), base_offset_ + TORQUE_ENABLE_OFFSET, &cmd, sizeof(char));
}

void PTUMotor::stopMotor(irl_can_bus::CANManager& can)
{
    static unsigned char cmd = 0;
    can.writeMem(deviceID(), base_offset_ + TORQUE_ENABLE_OFFSET, &cmd, sizeof(char));
}

void PTUMotor::setPoint(irl_can_bus::CANManager& can, double val)
{
    if (!motor_enabled_)
        return;
...
void PTUMotor::setMaxVel(irl_can_bus::CANManager& can)
{
    if (!motor_enabled_)
        return;
